...
public class OrchestrationMCPTest {
     ...
     * @param mcpServerCSDNJob MCP 测试服务。
     */
    @Autowired
    ...
    public void test_orchestration_with_mcp_tools() {
        ...
        // 测试工具调用
        // 重要使用明确的指令，让 AI 知道需要调用工具
        String userInput = """
                请使用 queryConfig 工具查询电脑配置信息。
                参数computer = "sxie47559"
        ...
        log.info(">>> 测试完成ChatClient 已自动注入 MCP 工具和 AgentEnhancers");
    }
     ...
     *
     * 说明：
     * - 编排层方式自动注入工具和 AgentEnhancers，代码简洁
     * - 直接调用方式需要手动注入，容易遗漏
     */
    @Test
    public void test_compare_orchestration_vs_direct() {
        log.info(">>> 对比测试编排层 vs 直接调用");
        ...
        log.info(">>> 编排层方式自动注入工具和 AgentEnhancers，代码简洁");

        // 方式 2直接调用（不推荐）
        // 需要手动注入 ToolCallbackProvider 和 AgentEnhancers
        // 容易遗漏，导致功能不完整
        log.info(">>> 直接调用方式需要手动注入，容易遗漏");

        log.info(">>> 结论推荐使用编排层方式，确保功能完整性和一致性");
    }
    ...
    public void test_orchestration_csdn_publish_tool() {
        ...
        String userInput = """
                我需要你帮我生成一篇文章，要求如下
                1. 场景为 AI 学习与实战系列文章
                2. 主题从以下列表中任选其一深入讲解，不要全部覆盖Spring AI + MCP 实战、RAG 入门、向量数据库实践、Skills 实战、Prompt Engineering、Embedding、微调与对齐（SFT/RLHF）、评测与安全、MLOps/上线、GPU/推理优化
                3. 文章结构清晰，按主题分小节，循序渐进，从概念、原理、关键步骤、实践示例与注意事项进行讲解
                4. 至少包含 8 个小节，每个小节不少于 400 字，全文不少于 5000 字
                5. 结尾给出学习路线与实践建议，便于新手跟学
                根据以上内容，不要阐述其他信息，请直接提供文章标题、文章内容、文章标签（最多7个，用英文逗号隔开）、文章简述（100字）
                将以上内容发布文章到CSDN。
                """;
    ...
    public void test_orchestration_csdn_weixin_notice_tool() {
        ...
        String userInput = """
                ...
                之后进行微信公众号消息通知，平台CSDN、主题为文章标题、描述为文章简述、跳转地址从发布文章到CSDN获取 url
                """;
    ...
    public void test_orchestration_csdn_weixin_notice_with_memory_tool() {
        ...
        log.info(">>> 测试完成CSDN 发布 + 微信通知（带聊天记忆）成功");
    }
