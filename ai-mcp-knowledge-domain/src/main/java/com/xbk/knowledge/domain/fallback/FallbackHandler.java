package com.xbk.knowledge.domain.fallback;
...
public class FallbackHandler {
     ...
     * @return 响应对象
     */
    public DomainAIResponse executeWithFallback(
            ModelConfig primary,
            List<ModelConfig> fallbacks,
            DomainAIRequest request) {
        ...
        if (circuitBreaker.isOpen(primary.getId())) {
            ...
            // 2. 尝试主模型（带重试）
            DomainAIResponse response = executeWithRetry(primary, request, DEFAULT_MAX_RETRIES);
        ...
        for (ModelConfig fallback : fallbacks) {
            ...
            DomainAIResponse response = executeWithRetry(fallback, request, DEFAULT_MAX_RETRIES);
     ...
     * @return 响应对象
     */
    private DomainAIResponse executeWithRetry(
            ModelConfig model,
            DomainAIRequest request,
            int maxRetries) {
