...
public interface McpGatewayAuthMapper extends BaseMapper<McpGatewayAuth> {
    ...
    /** 按主键 + org 删除鉴权记录（强 org 隔离） */
    int deleteGatewayAuthByIdAndOrgId(@Param("orgId") Long orgId, @Param("id") Long id);

    /** 按 API Key 精确查询鉴权记录 */
    McpGatewayAuth findByApiKey(String apiKey);

    /** 按 API Key + org 精确查询鉴权记录（强 org 隔离） */
    McpGatewayAuth findByApiKeyAndOrgId(@Param("orgId") Long orgId, @Param("apiKey") String apiKey);

    /** 按主键查询鉴权记录 */
    McpGatewayAuth findById(Long id);

    /** 按主键 + org 查询鉴权记录（强 org 隔离） */
    McpGatewayAuth findByIdAndOrgId(@Param("orgId") Long orgId, @Param("id") Long id);
