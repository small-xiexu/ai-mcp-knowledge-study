...
- ChatClient 组装：`ChatClientAssemblyService` + `ChatClientEnhancer`
...
### 6.5 对话记忆与 Redis（后端）
- 记忆实现：`ChatMemoryConfig` 装配 `RedisChatMemoryRepository`
- 过期策略：`ChatHistoryProperties.retentionDays` 控制 TTL（默认 7 天）
- 会话粒度：Job 场景显式传入 `conversationId`（如 `traceId`）
- Redis Key：统一在 `ChatRedisKeys` / `XxlJobRedisKeys` 管理
...
### 8.3 MCP 配置存储方式（当前版本）
- MCP Server 配置已入库（MySQL 表：`ai_mcp_server_config`）。
- 运行时由 `McpServerRuntimeService` 注册/更新/下线。
- 配置变更可通过接口触发热更新，无需重启。
...
### 9.3 MCP Client 配置来源（当前版本）
- MCP Client 配置从数据库读取，由运行时服务注册到 MCP Client。
- 变更后可调用 `POST /api/mcp/servers/refresh` 触发刷新。

### 9.4 MCP Server 启动前置条件
1. 构建 MCP 工具工程（如 `mcp-tool-csdn` / `mcp-tool-weixin`）
2. 确保 JAR 真实存在，路径可访问
3. 确保数据库中的 MCP Server 配置完整（端点/Headers/超时等）
...
- MCP Server 配置是否已启用且刷新成功（`/api/mcp/servers/refresh`）
...
- 数据库：MySQL + PostgreSQL（默认双数据源；仅用其一时需调整配置）
...
1. 创建模型配置
2. 创建任务类型
3. 调用 `/api/ai/chat` 发起一次请求
4. 查询指标 `/api/metrics/*`
5. 查询审计 `/api/audits/list`
...
4. **MCP 工具无法调用**
   - 检查 MCP Server 配置是否启用并已刷新（`/api/mcp/servers/refresh`）
...
### 17.3 MCP 配置文件模板（Streamable HTTP）
```json
{
  "mcpServers": {
    "mcp-tool-csdn": {
...
- MCP 工具日志：由 MCP Server 启动参数决定（数据库配置中的 command/args）
