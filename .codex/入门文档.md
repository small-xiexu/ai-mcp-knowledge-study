...
- `ai-mcp-knowledge-app/src/main/resources/application-dev.yml` 当前配置为 **PostgreSQL** 数据源。
...
- **统一管理多种大模型**（OpenAI / Anthropic / Gemini / Ollama 等），
...
graph TB
  subgraph UI[界面层/UI]
    WEB[前端 ai-mcp-knowledge-web]
  end

  subgraph InterfaceAdapters[接口适配层/Interface Adapters]
    TRIGGER[触发层 ai-mcp-knowledge-trigger]
  end

  subgraph Application[应用层/Application]
    APP_LAYER[应用层 ai-mcp-knowledge-application]
  end

  subgraph Domain[领域层/Domain]
    DOMAIN[领域层 ai-mcp-knowledge-domain]
  end

  subgraph Infrastructure[基础设施层/Infrastructure]
    INFRA[基础设施 ai-mcp-knowledge-infrastructure]
  end

  subgraph Boot[启动与装配/Boot]
    APP_BOOT[启动模块 ai-mcp-knowledge-app]
  end

  subgraph SharedKernel[共享内核/Shared Kernel]
    TYPES[通用类型 ai-mcp-knowledge-types]
    API[接口DTO ai-mcp-knowledge-api]
  end
...
## 4.5 DDD 学习专栏（从 0 到 1，结合本项目）

> 本节目标：让你在**本项目中完整掌握 DDD**，不仅理解概念，还能知道“代码具体在哪、应该怎么改、为什么这么分层”。

### 4.5.1 DDD 的核心思想（先建立直觉）

DDD（Domain-Driven Design）强调：
- **把复杂问题还原为业务领域**，而不是技术实现；
- **用统一语言沟通（Ubiquitous Language）**；
- **明确边界（Bounded Context）**；
- **模型驱动开发**：代码结构服务于领域模型，而不是数据库或框架。

> 简单理解：DDD 不是“写更多层”，而是“用业务语言组织代码”。

### 4.5.2 DDD 在本项目的落地结构

本项目按 DDD 分为 4 个核心层（经典分层）：
...
| Domain | ai-mcp-knowledge-domain | 实体、值对象、领域服务、仓储接口 |
| Infrastructure | ai-mcp-knowledge-infrastructure | 仓储实现、第三方模型调用 |

### 4.5.3 领域模型与业务语言（Ubiquitous Language）
...
这些名字会贯穿：
- API DTO（`ai-mcp-knowledge-api`）
- Domain Entity（`ai-mcp-knowledge-domain`）
- 数据表（`ai_model_config` / `ai_task_type` 等）
...
一组有强一致性约束的对象的边界。
在项目中：
- `ModelConfig` + `ModelCapability` 是典型聚合（模型配置及其能力）。
  - 由 `ModelConfig` 作为聚合根。
  - 能力配置随模型配置变化。

#### 4) Domain Service（领域服务）
当业务逻辑不适合放在实体中时，用领域服务承载。
项目中：
- `IModelConfigService`
- `ITaskTypeService`
...
DDD 中 Repository 是领域层对数据存储的抽象接口。
- 定义在 Domain
- 实现放在 Infrastructure
...
### 4.5.6 Application Service 与 Domain Service 的区别

| 类型 | 关注点 | 示例 |
| --- | --- | --- |
| Application Service | 用例编排、事务边界、跨聚合协作 | AIModelService / ModelSelector |
| Domain Service | 领域规则、业务约束 | IModelConfigService / ITaskTypeService |

在本项目中：
- AI 调用流程由 Application Service 驱动。
- 模型配置、任务配置属于 Domain Service。

### 4.5.7 Anti-Corruption Layer（防腐层）

当外部系统模型与内部模型不一致时，需要防腐层隔离。
在本项目中：
- Infrastructure Provider 是一种“防腐层”，将 OpenAI/Gemini/Anthropic 的调用转换为统一领域模型。
- Trigger 层的 DTOConverter 也是轻量防腐层，隔离接口与领域模型。

### 4.5.8 事务边界与一致性

DDD 的重点之一是“聚合内部强一致，聚合之间最终一致”。
本项目的典型事务边界：
- 创建模型配置时：`ModelConfig` + `ModelCapability` 同事务
- 更新任务类型时：只涉及 `TaskType`
- 调用日志与审计：通常在应用服务层统一写入

### 4.5.9 如何在项目中训练 DDD 思维

建议你按下面路径“边看边改”：

1. **先读 Controller**  
   看 `/api/models`、`/api/task-types`、`/api/ai` 入口  
...
- 涉及枚举（types）、策略选择逻辑（application）  

**练习 3：新增审计字段**  
- 目标：在审计记录中添加 `source` 字段  
- 涉及：Domain Entity + DB + Mapper + DTO + Controller

### 4.5.11 DDD 学习路线（建议）

- 第 1 阶段：掌握术语（实体、值对象、聚合根、领域服务、仓储）
- 第 2 阶段：能画出上下文边界与模块依赖图
- 第 3 阶段：能在代码中识别“应该属于哪层”
- 第 4 阶段：能独立扩展一条完整业务链路
...
### 6.1 AI 调用流程
1. 前端调用 `/api/ai/chat` 或 `/api/ai/chat/{taskType}`
2. Trigger 层接收请求，转换为应用层命令
3. Application 层选择模型（策略/任务优先）
4. Infrastructure Provider 负责调用具体模型 API
5. 结果回写：记录调用日志、返回响应
6. 如果模型失败，触发 fallback 降级
...
## 7. 数据库与表结构（含 PostgreSQL 版本）

### 7.1 MySQL 版本（项目自带）

脚本位置：`sql/init-ai-model-orchestration.sql`

### 7.2 PostgreSQL 版本（推荐使用此版本与当前配置一致）

> 注意：以下脚本默认数据库名为 `ai-rag-knowledge`，与 `application-dev.yml` 一致。如你要使用其他数据库名，请同步修改配置。
...
关键字段：
- `server.port`: 8090
- `spring.datasource.*`: 数据库连接（当前为 PostgreSQL）
...
#### 10.3.2 POST /api/ai/chat/{taskType}
说明：当路径指定 taskType，会优先按任务类型的首选模型选择。
...
#### 10.3.4 GET /api/ai/models/recommend?taskType=WRITING
返回推荐模型信息（ModelInfo）。
...
- **AI Playground**：`/api/ai/chat` / `chat/{taskType}`
- **指标看板**：`/api/metrics/*`
...
| taskType | string | 否 | 任务类型编码（如 WRITING/ANALYSIS） |
| systemPrompt | string | 否 | 系统提示词 |
| parameters | object | 否 | 模型参数（temperature/maxTokens 等） |
| strategy | string | 否 | 模型选择策略：QUALITY_PRIORITY / SPEED_PRIORITY / COST_PRIORITY |
...
### 18.2 调用 AI（指定任务类型）
```bash
curl -X POST http://localhost:8090/api/ai/chat/WRITING \\
  -H 'Content-Type: application/json' \\
  -d '{
...
### 18.3 模型列表
```bash
curl http://localhost:8090/api/ai/models
```
