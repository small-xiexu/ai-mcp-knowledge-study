...
### 4.1 编排模式选型（当前实现）
- `AiChatAppServiceImpl`：采用“模板方法式顺序编排”，按固定步骤准备上下文（模型、工具开关、提示词、日志骨架），再执行模型调用与落库，强调可读性和排障直观性。
- `WorkflowRuntimeAppServiceImpl`：采用“策略 + 工厂式分发”，按节点类型路由到对应执行器，强调节点扩展能力与运行时解耦。
- 责任链：当前不作为主编排容器，仅建议用于可插拔横切场景（如鉴权、限流、审计、灰度开关），避免把主业务流程拆散后难以追踪。
