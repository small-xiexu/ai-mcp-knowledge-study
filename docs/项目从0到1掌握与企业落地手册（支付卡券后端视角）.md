...
> 版本基线：基于当前仓库 `feat/multi-agent-platform-governance` 分支工作区（2026-02-23）  
...
- Prompt/Agent 增强器：`/api/templates`、`/api/agent-enhancers`
...
sequenceDiagram
    ...
    UI->>C: POST /api/ai/stream
    C->>S: AICallCommand
    S->>S: 解析模型 + 组装 Prompt(RAG/历史)
    S->>A: buildChatClient(model, tools)
    A->>M: 创建并调用 ChatClient
    M-->>S: Flux<ChatResponse>
    S->>DB: 保存调用日志/会话记忆
    S-->>C: 分片内容 + usage
    C-->>UI: text/event-stream
```

### 5.2 Agent 运行链路

`AgentRuntimeController -> AgentRuntimeAppService -> Agent/Version 解析 -> (普通链路 or Client 链路 or Planning 链路 or Workflow 转发) -> agent_run/agent_run_context`

要点：

- 按 `agentCode` 路由
- 版本绑定支持 4 种形态：Prompt / Chain / Planning / Workflow
- Planning 支持“自动规划 + 人工确认执行（可选）”
...
flowchart TD
    ...
    MODE -->|PLANNING| PLAN[自动生成执行计划]
    MODE -->|WORKFLOW| WF[转发 WorkflowRuntime 执行]
    PLAN --> CONFIRM{是否要求人工确认}
    CONFIRM -->|是| PLANPEND[写 approval_request<br/>approval_type=PLAN_EXECUTE]
    CONFIRM -->|否| PLANEXEC[按计划步骤执行]
    PLANPEND --> PLANRES[审批通过后 resumePlannedRun]
    PLANRES --> PLANEXEC
...
5. Agent 核心编排：`ai-mcp-knowledge-application/src/main/java/com/xbk/knowledge/application/service/app/impl/AgentRuntimeAppServiceImpl.java` `chat / resumePlannedRun`  
...
12. 审批续跑核心：`ai-mcp-knowledge-application/src/main/java/com/xbk/knowledge/application/service/app/impl/ApprovalAppServiceImpl.java` `approve`（含 `PLAN_EXECUTE`）  
...
7. Agent 运行面（重点）  
`IAgentRuntimeService` -> `AgentRuntimeController` -> `AgentRuntimeAppServiceImpl` -> `ChatClientAssemblyServiceImpl` + `GatewayToolCallbackProvider` + `DynamicMcpToolCallbackProvider` + Planning 编排 -> `AgentRunRepositoryImpl/AgentRunContextRepositoryImpl` -> `agent_run/agent_run_context`。
...
flowchart TD
    ...
    R1 --> AI[流式 AI 调用<br/>POST /api/ai/stream]
    ...
    RISK -->|否| OK
    RISK -->|是| AP0[创建审批单 approval_request<br/>状态 PENDING_APPROVAL<br/>approval_type=TOOL_INVOKE/PLAN_EXECUTE]
...
- C. Client/Agent增强器/Agent/Workflow/审批/运行：16 张
- 合计：37 张
...
#### C1. Agent增强器/Client/Agent（10）

- `agent_enhancer`：Agent 增强器资产；关键字段 `agent_enhancer_code`、`agent_enhancer_type`、`enabled`
- `agent_enhancer_binding`：Agent 增强器绑定关系；关键字段 `bind_type`、`bind_target_id`、`agent_enhancer_id`
...
- `agent_version`：Agent 版本快照；关键字段 `state`、`workflow_version_id`、`rag_mode`、`client_profile_id`、`planning_config_json`
...
- 关系类唯一：`(user_id, role_id)`、`(role_id, permission_id)`、`(tool_id, bind_type, bind_target_id)`、`(bind_type, bind_target_id, agent_enhancer_id)`
...
| `agent_enhancer`（Agent增强器资产库） | `id` / `agent_enhancer_code`(唯一) | `agent_enhancer_name,agent_enhancer_type,config_json` | `enabled,created_at,updated_at` | - |
| `agent_enhancer_binding`（Agent增强器绑定关系） | `id` / `(bind_type,bind_target_id,agent_enhancer_id)`(唯一) | `order_no` | `enabled,created_at,updated_at` | `agent_enhancer_id,bind_target_id` |
...
| `approval_request`（统一审批单） | `id` | `approval_type(TOOL_INVOKE/PLAN_EXECUTE),run_id,agent_id,agent_version_id,workflow_id,workflow_version_id,node_key,requester_id,requester_type,request_reason,approver_id,decision_comment,decided_at,tool_key,risk_level,arguments_snapshot_json,arguments_digest,expire_at` | `status,created_at,updated_at` | `run_id,agent_id,workflow_id,workflow_version_id` |
...
3. 给 Agent 增加 Planning 链路（自动规划 + 人工确认执行）。
4. 接入一套支付只读工具（订单、交易、退款查询）。
5. 接入一套卡券只读工具（券状态、活动规则查询）。
6. 新增监控看板指标并完成阈值告警配置。
...
2. 配 AgentVersion（运行模式建议 PLANNING）  
   - `planning_config_json` 建议：  
     `{"enabled":true,"requireHumanConfirm":true,"maxPlanSteps":6,"replanMaxTimes":1,"stepTimeoutMs":60000,"approvalExpireMinutes":120}`。  
   - `requireHumanConfirm=true` 时，先生成计划，再创建 `approval_type=PLAN_EXECUTE` 审批单。  
   - 计划执行阶段仅放行查询类 `toolKey`，写操作仍由工具风险审批兜底。
...
3. Planning 模式先自动产出步骤计划（目标、步骤、风险、预期产物）。  
4. 若要求人工确认，会进入 `PENDING_APPROVAL`，审批通过后 `resumePlannedRun` 继续执行。  
5. Agent 增强器按绑定顺序注入：先记忆，再日志，再工具调用日志。  
6. 模型如需查证，会调用被 allowlist 允许的查询工具（例如订单查询、交易流水查询）。  
7. 输出给客服的是“证据化结论”，不是一句模糊建议。  
8. 同时落地运行审计：`agent_run`、`agent_run_context`、工具调用日志、审批状态（若触发）。
...
2. 先配模板并发布：把系统提示词规范化。  
3. 再配 AgentVersion：明确运行模式（Prompt / Chain / Planning / Workflow）。  
4. 给每步配置工具权限：没有必要就关工具。  
5. 绑定 Agent 增强器：记忆、请求日志、工具日志至少三件套。  
